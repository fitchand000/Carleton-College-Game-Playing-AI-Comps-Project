...
class Trainer:
    ...
    def __init__(self, bot_count, bot_prefix='bot', print_rate=0, initialized_bots=[]):
        ...
        initialized_bots = list of bot names to use to initialize the new population
        """
    ...
    def train(self, mutation_percent, generations, games_per_bot, fast_count, bots_per_sim,
        ...
        operator_probability: a string integer under 100, probability of making an operator on a mutation (string number from 0 - 100)
        max_children: a string integer, the maximum number of descendants the node we are mutating is allowed to have (-1 for any number of children)
        constants_only: string "true" or "false", only mutate constant values in the tree
        performance_cutoff: float less than 1, the threshold between high performance and low performance robots
        high_performer_sample_rate: float less than 1, the proportion of the bots that come from the high performers when selecting for reproduction
        node_penalty: float, the number of points to deduct from a genetic tree for each node it has
        win_bonus: int, the number of bonus points a robot gets for winning
        last_gen: boolean, used to let the trainer know it should not mutate/crossover on the final generation
        delete_files, boolean, whether or not to delete the files generated by simulation.py
        """
        self.config_count += 1
